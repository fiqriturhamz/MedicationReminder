        ...
        GlobalScope.launch(dispatcher) {
            ...
            when (intent.action) {
                Intent.ACTION_BOOT_COMPLETED, Intent.ACTION_LOCKED_BOOT_COMPLETED, Intent.ACTION_MY_PACKAGE_REPLACED -> {
                    medications.forEach { medication ->
                        ...
                        if (medication.notify) {
                            ...
                            AlarmIntentManager.setExact(alarmManager,
                                alarmIntent,
                            ...
                            if (System.currentTimeMillis() > medication.calculateClosestDose().timeInMillis && !medication.closestDoseAlreadyTaken()) {
                                val notification =
                                    configureNotification(context, medication).build()
                ...
                TOOK_MED_ACTION -> {
                    ...
                    if (medicationDao(context).medicationExists(medId) && medicationDao(context).get(
                            medId).active
                    ) {
