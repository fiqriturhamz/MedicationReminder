...
@Database(entities = [Medication::class, ProofImage::class, MedicationType::class, DoseUnit::class],
    version = 9)
abstract class MedicationDB : RoomDatabase() {
    ...
    companion object {
        ...
        @Volatile
        private var instance: MedicationDB? = null
        ...
        private val MIGRATION_2_3 = object : Migration(2, 3) {
            override fun migrate(database: SupportSQLiteDatabase) {
                ...
                database.execSQL("ALTER TABLE $MED_TABLE ADD COLUMN startDay INTEGER DEFAULT ${
                    cal.get(Calendar.DAY_OF_MONTH)
                } NOT NULL")
                database.execSQL("ALTER TABLE $MED_TABLE ADD COLUMN startMonth INTEGER DEFAULT ${
                    cal.get(Calendar.MONTH)
                } NOT NULL")
                database.execSQL("ALTER TABLE $MED_TABLE ADD COLUMN startYear INTEGER DEFAULT ${
                    ...
                    object : Callback() {
                        override fun onOpen(db: SupportSQLiteDatabase) {
                            runBlocking {
                                launch {
                                    val undefinedType =
                                        MedicationType(context.getString(R.string.undefined))
                                    undefinedType.id = 0
                                    val defaultTypeExists =
                                        db.query("SELECT * FROM $MED_TYPE_TABLE WHERE id = 0").count > 0
                                    ...
                                    val undefinedUnit =
                                        DoseUnit(context.getString(R.string.undefined))
                                    undefinedUnit.id = 0
                                    val defaultUnitExists =
                                        db.query("SELECT * FROM $DOSE_UNIT_TABLE WHERE id = 0").count > 0
        ...
        fun databaseFileIsValid(context: Context, databaseUri: Uri?): Boolean {
            return try {
                ...
                val testDatabase =
                    Room.databaseBuilder(context, MedicationDB::class.java, TEST_DATABASE_NAME)
